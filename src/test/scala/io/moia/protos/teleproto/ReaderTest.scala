...
class ReaderTest extends UnitTest {
  ...
  "Reader" should {
    ...
    val reader = new Reader[DeliveryProto, DeliveryModel] {

      def read(protobuf: DeliveryProto): PbResult[DeliveryModel] =
        for {
          ...
          DeliveryModel(id, price, time, duration, pickupId, prices, discounts)
        }
    }
    ...
    "fail if value is missing" in {
      ...
      reader.read(
        DeliveryProto(None, Some("bar"), Some(Timestamp.defaultInstance), Some(PBDuration.defaultInstance), None, Seq.empty, Map.empty)
      ) shouldBe
    ...
    "read successfully if list is empty" in {
      ...
      reader.read(
        ...
        PbSuccess(DeliveryModel("foo", 1.2, Instant.ofEpochMilli(0), Duration.Zero, Some("pickup"), Nil, Map.empty))
    }
    ...
    "read successfully if an option is undefined" in {
      ...
      reader.read(
        ...
        PbSuccess(DeliveryModel("foo", 1.2, Instant.ofEpochMilli(0), Duration.Zero, None, List(1, 1.2, 1.23), Map.empty))
    }
    ...
    "read timestamps on nano level" in {
      ...
      reader.read(
        ...
        PbSuccess(DeliveryModel("foo", 1.2, Instant.ofEpochSecond(12, 34), Duration.Zero, None, Nil, Map.empty))
    }
    ...
    "read durations on nano level" in {
      ...
      reader.read(
        DeliveryProto(
          ...
          Some("1.2"),
          Some(Timestamp.defaultInstance),
          Some(PBDuration(12, 34)),
          None,
          Seq.empty,
          Map.empty
        )
      ) shouldBe
        PbSuccess(DeliveryModel("foo", 1.2, Instant.ofEpochMilli(0), 12.seconds + 34.nanos, None, Nil, Map.empty))
    }
  }
}
