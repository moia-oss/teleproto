...
class OneOfReaderTest extends WordSpec with Matchers with OptionValues {
  ...
  "Reader for one-of to sealed trait" should {
    ...
    implicit val fooReader: Reader[protobuf.Foo, model.Foo] =
      (p: protobuf.Foo) =>
        transform[String, BigDecimal](p.price, "/price").map(model.Foo)
    ...
    implicit val fooOrBarReader: Reader[protobuf.FooOrBar, model.FooOrBar] =
      (p: protobuf.FooOrBar) =>
        None
          .orElse(p.value.foo.map(foo =>
            transform[protobuf.Foo, model.Foo](foo, "/foo")))
          .orElse(p.value.bar.map(bar =>
            transform[protobuf.Bar, model.Bar](bar, "/bar")))
    ...
    val reader = new Reader[protobuf.Protobuf, model.Model] {
      ...
      def read(p: protobuf.Protobuf): PbResult[model.Model] =
        for {
          fooOrBar <- transform[protobuf.FooOrBar, model.FooOrBar](p.fooOrBar,
                                                                   "/fooOrBar")
        } yield {
